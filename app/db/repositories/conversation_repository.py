...
class ConversationRepository(BaseRepository[Conversation, ConversationCreate, ConversationUpdate]):
    ...
    async def get_by_id_for_user(self, id: UUID, user_id: UUID) -> Optional[Conversation]:
        """
        获取特定用户的特定会话
        """
        query = (
    ...
    async def update_files(self, conversation: Conversation, file_ids: List[UUID]) -> Conversation:
        """
        更新会话关联的文件
        """
        # 清空当前关联
